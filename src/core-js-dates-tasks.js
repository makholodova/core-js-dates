...
function getTime(date) {
  ...
  return resDate.toLocaleTimeString('it-IT');
}
...
function getNextFriday(date) {
  ...
  resDate.setDate(resDate.getDate() + daysUntilFriday);
  return resDate;
}
...
function getCountDaysInMonth(month, year) {
  const resDate = new Date(year, month, 0);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const resMs = end - start;
  return Math.floor(resMs / (1000 * 60 * 60 * 24)) + 1;
}
...
function formatDate(date) {
  ...
  return dataRes.toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  ...
  const daysSinceYearStartMs = date.getTime() - firstDayOfYear.getTime();
  let week = Math.ceil(daysSinceYearStartMs / oneWeekMs);
  week += firstDayOfYear.getDay() === date.getDay() ? 1 : 0;
  return week;
}
...
function getNextFridayThe13th(date) {
  ...
  while (!isFound) {
    fridayThe13th = new Date(year, month, 13);

    if (fridayThe13th.getDay() === 5) {
      isFound = true;
    }
    month += 1;
    if (month > 11) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDayMs = new Date(yearStart, monthStart - 1, dayStart).getTime();
  const endDayMs = new Date(yearEnd, monthEnd - 1, dayEnd).getTime();
  const dayInMs = 24 * 60 * 60 * 1000;
  const workSchedule = [];

  for (let i = startDayMs; i <= endDayMs; i += dayInMs * countOffDays) {
    let count = countWorkDays;
    while (count) {
      const formattedDate = new Date(i)
